...
3. **Le build écrit `dist/.canopui-version`** — la version réellement embarquée dans le
   bundle. Elle voyage avec `dist/` jusqu'au dossier servi (`/opt/custhome/ch-portail-admin/dist/`, recopié par le helper `ch-deploy-portal`).
...
2. **Corriger dans CanopUI** et republier (tag `vX.Y.Z`) : `latest` repart en avant, et le
   prochain démarrage — ou un `sudo canopui-autorebuild --force ch-portail-admin` — rattrape le
   front.
